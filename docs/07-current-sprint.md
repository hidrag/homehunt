...
## Status
[~] In progress — S4.0 (Architecture & Decision Lock) approved; S4.1 (implementation) not started.
...
## S4.1 — Implementation objectives (pending)
- [ ] Add `bcrypt` and `jsonwebtoken` (approved; stop and report before any substitution).
- [ ] `User` and `Session` models with the approved fields and indexes.
- [ ] Auth service: register (buyer only), login (no enumeration), refresh (strict rotation), logout (idempotent), me.
- [ ] `requireAuth` / `requireRole` middleware and session cookie helpers.
- [ ] Auth routes/controller per the `docs/05-api-contract.md` contract; dedicated auth rate limiter.
- [ ] Production-guarded dev user seed with fixed ids (agent `64b000000000000000000001`).
- [ ] Client: login/register forms (React Hook Form + Yup), auth state in Redux (user object only — tokens never leave cookies), single-flight refresh coordination in the Axios interceptor, UX-only route guard.
- [ ] Backend auth test suites; the 34 existing S1/S2 tests remain unmodified and green.
...
## Acceptance criteria (to be met by S4.1)
- [ ] Public registration always creates `buyer`; a supplied `role` is ignored.
- [ ] Login returns identical `401 INVALID_CREDENTIALS` for unknown email and wrong password.
- [ ] Access token 15 minutes, refresh token 7 days; both delivered as HttpOnly cookies per `docs/05-api-contract.md`.
- [ ] Refresh rotates strictly; reuse revokes the family and returns `401 REFRESH_TOKEN_REUSED`; logout is idempotent and clears both cookies.
- [ ] `requireAuth` returns `401 UNAUTHORIZED` for missing/invalid/expired access tokens; `requireRole` returns `403 FORBIDDEN`.
- [ ] Auth endpoints are rate limited (10 requests / 15 minutes / IP) with `429 RATE_LIMITED`.
- [ ] No password, token, or hash material appears in API responses or logs.
- [ ] Server `npm test` + `npm run lint` and client `npm run lint` + `npm run build` pass; the 34 existing tests are unmodified and green.
